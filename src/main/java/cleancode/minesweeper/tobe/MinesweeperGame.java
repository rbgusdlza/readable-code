...
public class MinesweeperGame {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            int selectRowIndex = getSelectRowIndex(cellInput);

            if (doesUserChooseToPlantFlag(userActionInput)) {
                BOARD[selectRowIndex][selectedColIndex] = FLAG_SIGN;
                checkIfGameIsOver();
            } else if (doesUserChooseToOpenCell(userActionInput)) {
                if (isLandMineCell(selectRowIndex, selectedColIndex)) {
                    BOARD[selectRowIndex][selectedColIndex] = LAND_MINE_SIGN;
    ...
    private static boolean isLandMineCell(int selectRowIndex, int selectedColIndex) {
        return LAND_MINES[selectRowIndex][selectedColIndex];
    }
    ...
    private static int getSelectRowIndex(String cellInput) {
    ...
    private static void checkIfGameIsOver() {
        boolean isAllOpened = isAllSellOpened(); //check로 시작하는 메서드는 주로 void 반환
        if (isAllOpened) {
    ...
    private static boolean isAllSellOpened() {
    ...
    private static void initializeGame() {
        for (int row = 0; row < LAND_MINE_COUNT; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
